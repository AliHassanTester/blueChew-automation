...
### Login Page — `tests/visual/login.visual.spec.ts`

**Setup:** Pass app dev gate → `/log-in`

| Test | Type |
|---|---|
| Login page — iPhone X snapshot | Snapshot |
| Login page — desktop 1440p snapshot | Snapshot |
| Login form component snapshot | Component snapshot |
| Log In button — color, font-weight tokens | Design token |
| Email input — font-size token | Design token |
| Password input — font-size token | Design token |

---

### Registration Page — `tests/visual/registration.visual.spec.ts`

**Setup:** Pass app dev gate → `/register` (step 1 directly)

| Test | Type |
|---|---|
| Registration step 1 — iPhone X snapshot | Snapshot |
| Registration step 1 — desktop 1440p snapshot | Snapshot |
| Registration step 1 form component snapshot | Component snapshot |
| CONTINUE button — color, font-weight tokens (after state + terms selected) | Design token |

---

### Quiz Page — `tests/visual/quiz.visual.spec.ts`

**Setup:** Pass quiz dev gate → `/quiz` → wait for first question

| Test | Type |
|---|---|
| Quiz question — iPhone X snapshot | Snapshot |
| Quiz question — desktop 1440p snapshot | Snapshot |
| Quiz answer button component snapshot | Component snapshot |
| Answer button — font-size, font-weight tokens | Design token |
| Progress indicator — font-size token | Design token |
| Body — font-family (Roboto) | Design token |

---

### Results Page — `tests/visual/results.visual.spec.ts`

**Setup:** Pass quiz dev gate → complete all 3 quiz questions → `/results`

| Test | Type |
|---|---|
| Results page — iPhone X snapshot | Snapshot |
| Results page — desktop 1440p snapshot | Snapshot |
| TRY GOLD button component snapshot | Component snapshot |
| TRY GOLD button — color, background, font-size, font-weight tokens | Design token |
| Page headings — color, font-size tokens | Design token |
