...
test.describe('Visual: Login Page (/log-in)', () => {
  ...
  test('Login page — iPhone X snapshot matches baseline', async ({ page }) => {
    await screenshotAtViewport(page, 'iphone-x', 'login-page-iphone-x');
  });

  test('Login page — desktop 1440p snapshot matches baseline', async ({ page }) => {
    await screenshotAtViewport(page, 'desktop-1440', 'login-page-desktop-1440');
  });
