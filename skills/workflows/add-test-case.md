...
1. Open `src/data/<priority>/<module>/<featureName>.data.ts`.
2. Add a new test data entry with a unique test case key.
3. Ensure the data key matches the `testCase` field value.
4. Open `src/specs/<priority>/<module>/<featureName>.spec.ts`.
5. Retrieve the new data with `getXxxData('TC-NEW-ID')`.
