// Starts from /checkout strength-selection and captures everything forward.
// Run: node scripts/capture-strength-forward.js
const { chromium } = require('@playwright/test');
const fs = require('fs');
const path = require('path');

const OUT = path.join(__dirname, '..', 'dom-snapshots', 'strength-forward');
...
async function capture(page, label) {
  ...
  if (btns.length) console.log(`  buttons:\n${btns.slice(0,10).map(b => `    id="${b.id}" cls="${b.cls?.slice(0,50)}" text="${b.text.slice(0,60)}" dtid="${b.dtid}"`).join('\n')}`);
  const inputs = data.elements.filter(e => ['input','select','textarea'].includes(e.tag));
  if (inputs.length) console.log(`  inputs:\n${inputs.slice(0,15).map(i => `    id="${i.id}" fcn="${i.fcn}" type="${i.type}"`).join('\n')}`);
  return data;
}
...
async function register(page) {
  const email = `aliQA.sf.${Date.now()}@example.com`;
...
async function completeQuiz(page) {
  const has = await page.waitForSelector('[data-test-id="quiz-answer-0"]', {timeout:5000}).then(()=>true).catch(()=>false);
  if (!has) {
...
async function fillMedical(page) {
  ...
  const fa = await page.locator('#fitness-agree-yes').waitFor({state:'visible',timeout:3000}).then(()=>true).catch(()=>false);
  if (fa) await page.locator('#fitness-agree-yes').check();
  ...
  for (let attempt=1; attempt<=2; attempt++) {
    ...
    const dm = page.locator('button:has-text("Dismiss")').first();
    if (await dm.isVisible().catch(()=>false)) {
      await dm.click(); await page.waitForTimeout(500);
      const ff = await page.locator('#fitness-agree-yes').isVisible().catch(()=>false);
      if (ff) { await page.locator('#fitness-agree-yes').check(); await page.waitForTimeout(300); }
    } else break;
  }
...
(async () => {
  ...
  // Slide 1: product intro — click div.slide-btn "CONTINUE"
  await page.waitForSelector('div.slide-btn', {timeout:10000});
  await page.locator('div.slide-btn').first().click();
  await page.waitForTimeout(1500);

  // Slide 2: strength selection — "High Strength" is pre-selected (Most Popular)
  // Walk up to 15 steps, never stopping on URL unchanged alone
  let lastBodyText = '';
  for (let step = 1; step <= 15; step++) {
    const url = page.url();
    const data = await capture(page, `${url.replace('https://dev.app.bluechew.com/','').replace(/\?.*/,'').replace(/\//g,'-')}-s${step}`);

    if (data.bodyText === lastBodyText && step > 1) { console.log('  Body unchanged — end of flow'); break; }
    lastBodyText = data.bodyText;
    ...
    for (const inp of data.elements.filter(e=>['input','select','textarea'].includes(e.tag)&&!e.disabled)) {
      ...
      else if (!['radio','hidden','submit','button'].includes(inp.type||'')) {
        const fcn=(inp.fcn||inp.id||'').toLowerCase();
        const val=fcn.includes('phone')?'3125550100':inp.type==='number'?'5':'TestValue';
    ...
    const ADVANCE=['continue','next','submit','proceed','confirm','checkout','place order','complete','pay','purchase','select'];
    let advanced=false;

    // Check div.slide-btn first
    ...
    if (!advanced) {
      for (const btn of data.elements.filter(e=>e.tag==='button'&&e.text&&!e.disabled)) {
        if (ADVANCE.some(t=>btn.text.toLowerCase().includes(t))) {
          const loc=btn.id?page.locator(`#${btn.id}`).first():page.locator('button').filter({hasText:btn.text.slice(0,30)}).first();
    ...
    if (!advanced) { await clickActiveStepContinue(page).catch(()=>{}); advanced=true; }
  ...
  await browser.close();
  console.log('\n✅ Done — dom-snapshots/strength-forward/');
})();
